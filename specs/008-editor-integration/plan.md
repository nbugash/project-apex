...
|---|---|---|
| Chunk threshold (FR-016, FR-018) | **512 KiB**, as `wire::MAX_INLINE_READ` | A file that fits in one frame costs one round trip whole; ranging it costs the same round trip and delivers less, so below the threshold ranging is strictly worse. **Corrected during implementation from 1 MiB.** The original reasoning named §4.1's 1 MiB frame cap and stopped there: content travels base64, four bytes out for every three in, so 1 MiB raw encodes to roughly 1.37 MiB and fails on the first frame. A-BULKSIZE had already fixed the real threshold at 512 KiB for exactly this reason. Referenced as the constant rather than restated as a number, so there is one of it (Principle II). |
...
| Maximum write in one frame | **the encoded params, under `MAX_FRAME_BYTES` less 4 KiB of envelope** | Added during implementation. `writeFile` carries content as a JSON *string*, where a quote costs two bytes and a control character six, so no limit predicted from the raw byte count is right for the files most likely to reach it. The client measures the encoded frame and refuses before sending; the engine keeps its own independent `MAX_WRITE_BYTES` for a client that does not (Principle VI). Without this the one permanently-failing save is the one reported as "try again". |
