...
- [X] T088 Run `npm run lint:ds` and `npm run gate:fidelity` against the three new interface surfaces — `client/ui/lib/workspace/FileTree.svelte`, `VerifyBadge.svelte` and `MaintenanceBanner.svelte` — confirming tokens only, no raw hex, no raw pixel values (Principle I)
