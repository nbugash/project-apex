...
- [X] T090 Tick F003's six subfeature boxes in `specs/features-map.md` **only** for what shipped, and reconcile the "local and remote implementations" line with plan.md's Structure Decision — the local provider belongs to F015 and is deliberately not built here. Do not tick a box for work that did not happen
