...
- [ ] T032 [P] [US1] Contract test in `engine/tests/file_events.rs` for `workspace/onFileEvent`: the array shape, the four event kinds, `to_path` present only on `renamed`, `type`/`size`/`modified` present on `created` and `modified`, and **no bytes and no hash on any kind** (FR-010, FR-013)
