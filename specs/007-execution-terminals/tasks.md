...
- [ ] T032a Widen `engine/src/application/ports/clock.rs` to `Clock: Send + Sync` and add `fn sleep_until(&self, deadline: Millis)` — the escalation cannot wait against a port whose only method is `now()`, and the chunker reads the clock on N reader threads while the stop path reads it on the dispatch thread, which `Send`-without-`Sync` cannot support (design.md [CONFLICT 8])
- [ ] T032b Move `engine/tests/common/fake_clock.rs` from `Cell<Millis>` to `Mutex<Millis>` plus a `Condvar`, so `advance` notifies and `sleep_until` waits until `now >= deadline` — a `Cell`-backed fake is actively `!Sync` and cannot be shared with the thread doing the waiting, which is what T074 and T092 assume
- [ ] T032c Add the single escalation thread in `engine/src/adapters/outbound/task_threads.rs` owning a deadline set: `TaskService` records a deadline when it sends `SIGTERM`, the thread wakes and sends `SIGKILL` to the process group of any task still alive at its deadline. One thread for the engine, not one per stop. The dispatch thread writes the `terminate` response immediately and never waits (FR-012, FR-017, §1.4)
